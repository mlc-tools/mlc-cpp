...
SerializerBase::SerializerBase() : model(nullptr), currentClass(nullptr) {}
...
void SerializerBase::generateMethods(Model &m) {
    ...
    for (auto &fmt : SerializeFormat_getAll()) {
        ...
        for (auto &clsPtr : m.classes) {
            ...
            createSerializationFunction(
                *currentClass, SerializationType::DESERIALIZATION, name);
        }
    }
}
...
void SerializerBase::createSerializationFunction(Class &cls,
    ...
    std::string methodName =
        (t == SerializationType::SERIALIZATION ? "serialize_"
                                               : "deserialize_") +
        format;
    ...
    if (!cls.parent_class_name.empty() && cls.type != "enum" &&
        ...
        //         use replace_all
        body += format_indexes(getParentSerializeCall(), cls.parent_class_name,
                               methodName, fn.callable_args.at(0).name);
    }
    ...
    for (auto &member : cls.members) {
        ...
        body += buildSerializeOperation(
            member.name, member.type, member.value, t, member.template_args,
            member.is_pointer, member.is_link, format);
    }
