...
protected:
    ...
    std::vector<std::pair<std::shared_ptr<Class>, std::pair<std::string,std::string>>> files_;
    ...
    std::pair<std::string,std::string> getConstructorData(const std::shared_ptr<Class> &cls);

    // Функции
    virtual std::pair<std::string,std::string> writeFunction(const Function &method);
