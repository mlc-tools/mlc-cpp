...
void GeneratorOperatorEqualsBase::generate(Model &model) {
    ...
    for (auto &cls : model.classes) {
        if (cls->type == "enum")           continue;
        if (cls->is_inline)                continue;
        if (cls->name == "DataStorage")    continue;
...
Object GeneratorOperatorEqualsBase::getConstRef(const std::shared_ptr<Class> &cls, const std::string& name) {
    Object ref;
    ref.type   = cls->name;
    ref.name = name;
    ref.is_const= true;
    ...
    const std::shared_ptr<Class> &cls)
{
    ...
    for (const auto &member : cls->members) {
        if (member.is_static || member.is_const
            || member.type == "Observable"
            || member.name == "_reference_counter")
            continue;
        bool skip = false;
        for (const auto &t : member.template_args) {
            if (t.type == "Observable") { skip = true; break; }
        }
        if (skip) continue;
