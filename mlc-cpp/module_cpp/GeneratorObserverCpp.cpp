...
const std::string PREDEFINED = R"___P(
...
void GeneratorObserverCpp::generate_file(Model& model)
{
    auto cls = generate_class();
    
    auto text = PREDEFINED;
    auto filename = cls->name + ".h";
    replace_all(text, "@{namespace}", "mg");

    model.addFile(cls, filename, text);
    model.add_class(cls);
