...
//    }
    ...
    // Return files
    return {
        { getFilename(cls,"h"), prepareFile(header) },
        { getFilename(cls,"cpp"), prepareFile(source) }
    };
}
...
std::pair<std::string,std::string> WriterCpp::writeFunction(const Function &fn)
{
    ...
    for (auto &arg : obj.template_args) {
        ...
        if (!arg.callable_args.empty() || obj.type == "Observable") {
            ...
            for (auto &c : arg.callable_args) {
                callParts.push_back(WriterCpp::writeNamedObject(c, "", true, false));
            }
            s += "(" + join(callParts, ", ") + ")";
        }
        parts.push_back(s);
...
std::string WriterCpp::writeNamedObject(
        const Object &obj,
        ...
        bool useIntrusive)
{
    ...
    if (posL!=std::string::npos && posR!=std::string::npos && posR>posL) {
        ...
        baseType = baseType.substr(0,posL+1) + join(toks,", ") + baseType.substr(posR);
    }
    bool usePtr = useIntrusive && obj.is_pointer && !obj.is_const && !obj.is_link && !obj.denied_intrusive;
    std::string pattern = usePtr
        ? "{static}{const}intrusive_ptr<{type}{templates}>{ref}{name}"
...
std::string WriterCpp::writeMemberEnumDeclaration(const Object &obj) {
    ...
    return "static constexpr BaseEnum " + obj.name
         + " = " + obj.value + ";";
}

// Static member initialization in the CPP
std::string WriterCpp::writeMemberStaticInit(
        const Class &cls,
        const Object &obj)
{
    bool useIntr = true;
    std::string tpl = getTemplates(obj);
    std::string init = obj.value.empty()
        ? ""
        : "(" + obj.value + ")";
    std::string patPtr = "{const}intrusive_ptr<{type}{templates}> {owner}::{name}{initial}"
                         ";";
    std::string patNon = "{const}{type}{templates}{pointer} {owner}::{name}{initial}"
                         ";";
    std::string pat = (useIntr && obj.is_pointer && !obj.is_const && !obj.is_link && !obj.denied_intrusive)
        ? patPtr : patNon;
    // fill
...
std::set<std::string> WriterCpp::getIncludesForMethod(
    const std::shared_ptr<Class> &cls,
...
std::string WriterCpp::buildIncludes(
    const std::shared_ptr<Class> &cls,
    const std::set<std::string> &incs)
{
...
std::string WriterCpp::buildForwardDeclarations(
    const std::set<std::string> &decls)
{
    static const std::set<std::string> ignore = {
        "std::vector","std::map","std::set","std::string",
        "int","bool","float","void","Observable"
    };
...
// Filename helpers
std::string WriterCpp::getFilename(
    const std::shared_ptr<Class> &cls,
...
std::string WriterCpp::getPathToRoot(
    const std::shared_ptr<Class> &cls)
{
