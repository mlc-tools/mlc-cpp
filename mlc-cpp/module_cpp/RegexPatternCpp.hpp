...
public:
    ...
    static inline std::vector<RegexPattern> convertC17toC14 = []{
        ...
        v.push_back({
            std::make_unique<RE2>(R"(for\s*\(auto&&\s*\[(\w+),\s*(\w+)\]\s*:\s*(.+)\)\s*\{)"),
            R"(for (auto&& pair : \3)
{
    auto& \1 = pair.first;
    auto& \2 = pair.second;
            ...
            R"(for(int __index__ = 0; __index__ < \1.size(); ++__index__)
{
    ...
    \3;
        ...
        v.push_back({
            std::make_unique<RE2>(R"(throw new Exception\((.*?)\))"),
            R"(throw std::exception(\1);)",
            { "throw " }
        });
        v.push_back({
            std::make_unique<RE2>(R"(\bnew\s*(\w+)\s*\((.*)\))"),
            R"(make_intrusive<\1>(\2))",
            { "new" }
        });
        v.push_back({
            std::make_unique<RE2>(R"((\w+)\*\s+(\w+)\s*=\s*new\s*(\w+)\s*\(\s*\))"),
        ...
        v.push_back({
            std::make_unique<RE2>(R"(std::strcat\((.+?),\s*(.+?)\))"),
            R"((std::string(\1) + std::string(\2)))",
            { "std::strcat" }
        });
        v.push_back({
            std::make_unique<RE2>(R"(std::std::)"),
            "std::",
            { "std::std" }
        });
        v.push_back({
            std::make_unique<RE2>(R"(([\w\->\.]+)\s*=\s*([\w\->\.]+)\s*\?\?\s*([\w\->\.]+))"),
            R"(\1 = (\2 != nullptr) ? \2 : (\3))",
            { "??" }
        });
        v.push_back({
            std::make_unique<RE2>(R"(\bprint\s*\(\s*(.+)\);)"),
            R"(std::cout << format(\1) << std::endl;)",
            { "print" }
        });
        v.push_back({
            std::make_unique<RE2>(R"(try\n\s*\{([\s\S]+?)\}\n\s*catch\(((\w+)\s*(\w*))\)\n\s*\{([\s\S]+?)\})"),
            R"(try
{\1}
