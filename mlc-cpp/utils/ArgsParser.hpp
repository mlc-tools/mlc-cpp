...
public:
    ...
    ArgParser& addFlag(std::string long_name, char short_name, std::string help,
                       bool required = false);
    ArgParser& addOption(std::string long_name, char short_name, std::string help,
                         bool multi = false, std::string default_value = "",
                         bool required = false);
    ...
    std::vector<std::string> getAll(const std::string& long_name) const; // все значения
...
private:
    ...
    std::unordered_map<char, std::string> _short_to_long;  // отображение short -> long
