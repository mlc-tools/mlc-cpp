...
ArgParser &ArgParser::addFlag(std::string long_name, char short_name,
    ...
    OptionSpec spec{std::move(long_name),  short_name,
                    /*takes_value*/ false, /*multi*/ false,
                    std::move(help),       /*default_value*/ "",
                    /*required*/ required};
    OptionState st{spec};
    _short_to_long.erase(spec.short_name);
    _options[spec.long_name] = std::move(st);
    if (short_name)
        _short_to_long[short_name] = spec.long_name;
    return *this;
}

ArgParser &ArgParser::addOption(std::string long_name, char short_name,
                                std::string help, bool multi,
                                std::string default_value, bool required) {
    OptionSpec spec{std::move(long_name), short_name,
                    /*takes_value*/ true, multi,
                    std::move(help),      std::move(default_value),
                    /*required*/ required};
    OptionState st{spec};
    if (!spec.default_value.empty())
        st.values.push_back(spec.default_value);
    _short_to_long.erase(spec.short_name);
    _options[spec.long_name] = std::move(st);
    if (short_name)
        _short_to_long[short_name] = spec.long_name;
...
ArgParser::OptionState *ArgParser::findByShort(char name) {
    ...
    if (it == _short_to_long.end())
        return nullptr;
    return findByLong(it->second);
}

const ArgParser::OptionState *
ArgParser::findByLong(const std::string &name) const {
    auto it = _options.find(name);
    return it == _options.end() ? nullptr : &it->second;
}
...
bool ArgParser::parse(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (starts_with(token, "--")) {
            ...
            if (!st->spec.takes_value) {
                if (!value.empty()) {
                    _error = "Опция --" + name + " не принимает значение";
                    return false;
                }
                continue;
            }
            ...
            if (value.empty()) {
                ...
                if (i + 1 >= argc) {
                    _error = "Для --" + name + " требуется значение";
                    return false;
                }
                value = argv[++i];
            }
        ...
        // Короткие флаги: -w или -o value. Комбинации -abc трактуем как -a -b
        // -c (только флаги).
        if (starts_with(token, "-")) {
            if (token.size() < 2) {
                _error = "Пустой короткий флаг '-'";
                return false;
            }
            // Разбираем пачку: -abc
            for (size_t k = 1; k < token.size(); ++k) {
                ...
                if (!st) {
                    _error = std::string("Неизвестная опция: -") + ch;
                    return false;
                }
                st->present = true;
                if (st->spec.takes_value) {
                    // Значение может идти слитно (-oValue) или отдельным
                    // аргументом (-o Value)
                    std::string val;
                    if (k + 1 < token.size()) {
                    ...
                    break; // короткая опция со значением — завершаем текущий
                           // токен
                }
            }
            continue;
    ...
    // Проверка обязательных опций: требуем, чтобы они были явно указаны в
    // командной строке
...
std::string ArgParser::get(const std::string &long_name,
...
std::string ArgParser::help(const std::string &program_name) const {
    ...
    for (const auto &lname : names) {
        ...
        if (st.spec.required)
            required_rows.push_back(std::move(row));
        else
            optional_rows.push_back(std::move(row));
    }
