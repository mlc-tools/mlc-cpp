...
bool write(const std::string &path, const std::string &content) {
    ...
    // Fast path: if file exists and size matches, compare bytes; otherwise skip
    // read.
        ...
        try {
            ...
            if (sz == content.size()) {
                ...
                if (in) {
                    ...
                    if (in.good() && std::memcmp(buf.data(), content.data(),
                                                 buf.size()) == 0) {
                        return false;
                    }
                }
