...
WriterPython::writeClass(const std::shared_ptr<Class> &cls)
{
    ...
    for (size_t i=0;i<slots.size();++i){ slots_line += slots[i]; if(i+1<slots.size()) slots_line += ", "; }
    slots_line += "]";
    ...
    if (!cls->parent_class_name.empty())
    {
        ...
        imports += "from ." + superclass_name + " import " + superclass_name + "\n";
...
WriterPython::writeObject(const Object &obj)
{
    ...
    if (value.empty() && !o.is_pointer)
    {
        ...
        if (type_ == "string") value = "\"\"";
        if (type_ == "int") value = "0";
        if (type_ == "float") value = "0";
        if (type_ == "uint" || type_ == "unsigned") value = "0";
        if (type_ == "bool") value = "False";
        if (type_ == "list") value = "[]";
        if (type_ == "map") value = "{}";
        if (value.empty()) {
    ...
    if (_model->hasClass(o.type) && currentClass_->name != o.type && o.type != currentClass_->parent_class_name)
...
{
    // Build pythonic args: 'self' unless static, then each arg name or name=default
...
std::string WriterPython::getNullptrString() const
{
    return "None";
}

std::string WriterPython::addStaticModifierToMethod(const std::string &text) const
{
...
std::string WriterPython::preparePythonFile(const std::string &text) const
{
    ...
    {
        out += result[i];
        if (i + 1 < result.size()) out.push_back('\n');
    }
    return out;
}
