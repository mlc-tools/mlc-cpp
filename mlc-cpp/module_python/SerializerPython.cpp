...
std::string SerializerPython::getProtocolText(const std::string & /*format*/) {
    // Python runtime serializers are imported in files; generation just calls
    // them.
    return "";
}

void SerializerPython::createSerializationFunction(Class &cls,
                                                   SerializationType t,
...
Object SerializerPython::getSerializationFunctionArg(
    SerializationType t, const std::string & /*format*/) const {
    (void)t;
...
std::string SerializerPython::buildSerializeOperation(
    ...
    if (t == SerializationType::SERIALIZATION) {
        std::string value = fieldValue.empty()
                                ? std::string()
                                : ", " + convertInitializeValue(fieldValue);
        return "\n        serializer.serialize(self." + fieldName + ", \"" +
               fieldName + "\"" + value + ")";
    }
    // Build meta and local imports for deserialization based on the field
    // itself
    ...
    std::string defv = fieldValue.empty()
                           ? std::string()
                           : ", " + convertInitializeValue(fieldValue);
    return std::string("\n") + imports_text + "        self." + fieldName +
           " = serializer.deserialize(\"" + fieldName + "\", " + meta + defv +
           ")";
}
...
std::string
SerializerPython::convertInitializeValue(const std::string &value) const {
    if (value.empty())
        return value;
    if (value == "true")
        return "True";
...
SerializerPython::create_meta_class(const Object &obj) const {
    ...
    if (obj.is_link) {
        return {"Meta(DataWrapper, " + obj.type + ")",
                {"DataWrapper", obj.type}};
    } else if (obj.is_pointer) {
        return {"Meta(IntrusivePtr, " + obj.type + ")",
                {"IntrusivePtr", obj.type}};
    ...
    if (model && model->hasClass(obj.type))
        imports_accum.push_back(obj.type);
