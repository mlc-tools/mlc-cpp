...
std::string AccessSpecifierToString(AccessSpecifier value) {
    if (value == AccessSpecifier::m_public)
        return "public";
    if (value == AccessSpecifier::m_protected)
        return "protected";
    if (value == AccessSpecifier::m_private)
        return "private";
    assert(0);
    return "";
}
...
bool Object::is_equal(const Object &other) const {
    ...
    if (result) {
        for (size_t i = 0; i < template_args.size(); ++i) {
            result = result &&
                     template_args.at(i).is_equal(other.template_args.at(i));
        }
    }
    if (result) {
        for (size_t i = 0; i < callable_args.size(); ++i) {
            result = result &&
                     callable_args.at(i).is_equal(other.callable_args.at(i));
        }
    }
    return result;
}
...
void Object::set_modifier(const std::string_view &modifier) {
    if (modifier == Modifier::m_private)
        access = AccessSpecifier::m_private;
    else if (modifier == Modifier::m_protected)
        access = AccessSpecifier::m_protected;
    else if (modifier == Modifier::m_public)
        access = AccessSpecifier::m_public;
        ...
        this->is_ref = true;
    else if (modifier == Modifier::m_runtime)
        this->is_runtime = true;
    else if (modifier == Modifier::m_static)
        this->is_static = true;
    else if (modifier == Modifier::m_const)
        this->is_const = true;
    else if (modifier == Modifier::m_key)
        this->is_key = true;
...
void Object::set_default_initial_value() {
    ...
    if (!value.empty())
        return;
    if (type == Objects::INT.type || type == Objects::UINT.type ||
        type == Objects::INT_64.type || type == Objects::UINT_64.type) {
        value = "0";
