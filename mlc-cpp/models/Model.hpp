...
class Model {
  public:
    using FileEntry =
        std::tuple<std::shared_ptr<Class>, std::string, std::string>;
    ...
    // Работа с файлами
    void addFile(const std::shared_ptr<Class> &cls,
    ...
    getSubclassesOfClass(const std::string &superName) const;
    ...
    // именами классов.
    void remove_classes_from_source(
  ...
  public:
    ...
    std::unordered_map<std::string, std::vector<std::string>>
        source_to_classnames;
    // Набор «грязных» классов для инкрементальной генерации. Пусто = генерить
    // все
