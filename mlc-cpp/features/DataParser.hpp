...
class DataParser {
public:
    DataParser(Model &model, const std::vector<std::shared_ptr<Class>> &classes,
...
private:
    struct XmlItem {
        ...
        std::unique_ptr<pugi::xml_document>
            doc; // holds single root element ('value')
    };

    void parseDirectory(const std::string &directory);
    void parseXml(const std::string &full_path);
    void parseJson(const std::string &full_path);
    void parseJsonNode(const std::string &full_path, const std::string &key,
