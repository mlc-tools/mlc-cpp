...
static std::string pluralizeWord(const std::string &w) {
    ...
    if (ends_with("s") || ends_with("x") || ends_with("z") || ends_with("ch") ||
        ends_with("sh"))
        return w + "es";
    static const std::unordered_set<std::string> f_exceptions = {
        "roof",  "belief", "chef", "chief", "cliff",
        "proof", "reef",   "safe", "gulf"};
    if (ends_with("fe"))
        return w.substr(0, w.size() - 2) + "ves";
    if (ends_with("f") && !f_exceptions.count(w))
        return w.substr(0, w.size() - 1) + "ves";
    static const std::unordered_set<std::string> o_es = {
        "hero", "potato", "tomato", "echo", "veto", "torpedo", "embargo"};
    if (ends_with("o"))
...
void GeneratorDataStorageBase::createGetters(
...
void GeneratorDataStorageBase::createGettersMaps(
    const std::vector<std::shared_ptr<Class>> &classes) {
    for (auto &cls : classes) {
        if (!cls->is_storage)
            continue;
